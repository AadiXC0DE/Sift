...
impl Conn {
    ...
    async fn reconnect(&mut self) -> Result<(), SiftError> {
        ...
        self.io = fresh.io;
        *self.inner.caps.write().await = pool.inner.caps.read().await.clone();
        Ok(())
    }
    ...
    pub async fn append(
        ...
        // LITERAL+ form: no continuation wait (Gmail advertises LITERAL+).
        let head = format!(
            "APPEND {}{flag_str} {{{}+}}\r\n",
            quote_folder(folder),
            bytes.len()
        ...
        // Read until the tagged completion.
        loop {
            match self.read_response().await? {
                Response::TaggedOk { code, .. } => {
                ...
                Response::TaggedNo { text, .. } | Response::TaggedBad { text, .. } => {

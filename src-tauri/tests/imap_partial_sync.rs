...
async fn p11_t06_trash_untrash_delete() {
    ...
    // Deterministic single-message thread (thrid == msgid, smallest id) so
    // `changed` carries the same hex as the message row; HashMap iteration
    // is random and multi-message threads would report the thread id instead.
    let (target, thrid_hex): (u64, String) = {
        let st = ctx._fake.state.lock().unwrap();
        let mut cands: Vec<(u64, u64)> = st
            .msgs
            .iter()
            .filter(|(_, m)| m.folders.contains_key("all") && !m.labels.contains(&"DRAFT".into()))
            .map(|(id, m)| (*id, m.thrid))
            .collect();
        cands.sort_unstable();
        // Prefer a single-message thread for a stable changed-thread assertion.
        let (id, th) = cands
            .iter()
            .find(|(id, th)| id == th)
            .copied()
    ...
    let (changed, _) = synced(tick(&ctx, false).await);
    assert!(
        changed.iter().any(|(_, t)| *t == thrid_hex),
        "trash should surface thread {thrid_hex} (msg {hex}): {changed:?}"
    );

// Trusted classic script loaded by opaque-origin email frames.
(function () {
  ...
  function post(m) {
    parent.postMessage({ __sift: true, token: TOKEN, ...m }, '*');
  }
  function report() {
  ...
  document.addEventListener('click', function (e) {
    ...
    if (a) {
      e.preventDefault();
      post({ type: 'link', href: a.getAttribute('href'), text: a.textContent.slice(0, 120) });
      return;
    }
  });
  var hoverT = null;
  document.addEventListener('mouseover', function (e) {
    ...
    if (a) {
      ...
      hoverT = setTimeout(function () {
        post({ type: 'hover', href: a.getAttribute('href') });
      }, 300);
    }
  });
  ...
  function activate(idx) {
    ...
    if (idx >= 0 && findMarks[idx] && findMarks[idx].scrollIntoView) {
      try {
        findMarks[idx].scrollIntoView({ block: 'center' });
      } catch (e) {}
    }
  }
